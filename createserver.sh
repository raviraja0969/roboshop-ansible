...
do
    ...
    val=$(aws route53 list-resource-record-sets --hosted-zone-id Z07109972OIC1QY2FF7I9  --query "ResourceRecordSets[?Name == '$i.$DOMAIN_NAME.']" | grep Name | wc -l)
    if [ $val -gt 0 ];
    then
        ACTION="UPSERT"
    else    
     ...
     aws route53 change-resource-record-sets --hosted-zone-id Z07109972OIC1QY2FF7I9 --change-batch '
     {
            "Changes": [{
            "Action": "'$ACTION'",
                        "ResourceRecordSet": {
